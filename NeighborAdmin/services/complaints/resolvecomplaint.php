  ...
  if(isset($params)){
    ...
    if(isset($connex)){
      ...
      if(isset($resultado1)){
        ...
        if($existe == 1){
          $sql3 = 'update dbneigh_admin.tcomplaints set isresolved = '.$jsonParams->isresolved
          ...
          if($filasact > 0){
            $salida = '{"res": 1, "msg":"QUEJA RESUELTA ACTUALIZADO ('.$jsonParams->idusr.')"}';
          }else{
            $salida = '{"res": 0, "msg":"NO SE PUDO RESOLVER LA QUEJA"}';
          }
        }
      }
    }
