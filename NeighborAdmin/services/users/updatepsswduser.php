  ...
  if(isset($params)){
    ...
    if(isset($connex)){
      ...
      if(isset($resultado1)){
        ...
        if($existe == 1){
          $sql3 = 'update dbneigh_admin.tusers set psswd = password("'.$jsonParams->psswd
          .'"), dupd = current_timestamp where idusr = '.$jsonParams->idusr.' ';
          $resultado4 = $connex->query($sql3);
          $filasact = $connex->affected_rows;
          if($filasact > 0){
            $salida = '{"res": 1, "msg":"CONTRASENA ACTUALIZADA DEL USUARIO CORRECTAMENTE"}';
          }else{
            $salida = '{"res": 0, "msg":"NO SE PUDO ACTUALIZAR LA CONTRASENA DEL USUARIO"}';
          }
        }
      }
    }
